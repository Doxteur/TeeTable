...
import { executeCustomQuery, fetchTableData, setTableData } from '../app/reducers/Tables';
...
function ResultList() {
  ...
  const total_count = useSelector((state) => state.tables.totalCount);
  ...
  useEffect(() => {
    if (db && selectedTable) {
      dispatch(fetchTableData({ db, selectedTable }));
    }
  }, [db, selectedTable, dispatch]);

  const handleCustomQuery = (customSql) => {
    if (db && customSql) {
      dispatch(executeCustomQuery({ db, sql: customSql }));
    }
  };

  // Filter  start with SELECT * FROM table WHERE
  const createCustomQuery = () => {
    let customQuery = 'SELECT * FROM ' + selectedTable + ' WHERE ';
    if (filterValue) {
      customQuery += filterValue;
    }
    handleCustomQuery(customQuery);
  };

  useEffect(() => {
    // if error react toastify
    if (error) {
      toast.error(error);
    }
  }, [error]);
      ...
      <div className='grid md:grid-cols-2 gap-y-2 md:gap-y-0 md:gap-x-5'>
        <div>
          ...
          <div className='relative'>
            ...
            <div className='absolute inset-y-0 -end-14 flex items-center  z-20 pe-1 bg-secondary rounded-lg p-1 cursor-pointer'>
              <button
                type='button'
                className='text-success '
                onClick={createCustomQuery}
                aria-label='Close'
              >
                Send
              </button>
                        ...
                        <th key={field} scope='col' className='min-w-[150px]'>
                          <div className='hs-dropdown relative inline-flex w-full cursor-pointer'>
                            ...
                            >
                              ...
                              <svg
                                className='shrink-0 size-3.5 ml-auto'
                                xmlns='http://www.w3.org/2000/svg'
                                width={24}
                                height={24}
                                viewBox='0 0 24 24'
                ...
                <tbody className='divide-y divide-stone-200 dark:divide-neutral-700'>
                  {sortedData.map((row, index) => (
                    <tr key={index}>
                      {fields &&
                        fields.map((field) => (
                          <td key={field} className='size-px whitespace-nowrap px-4 py-1'>
